...
#pragma omp parallel for
    ...
    void PatchMatch::spatial_propagation(int x, int y, int cpv, int iter) {
        ...
        for(auto it  = offsets.begin(); it < offsets.end(); ++it)
        {
            ...
            if(!inside(nx, ny, 0, 0, cols_, rows_))
                continue;

            // perturb the current disparity and test it
            float new_disp = disps_[cpv](ny, nx);
    ...
    void PatchMatch::view_propagation(int x, int y, int cpv) {
        ...
        // current plane
        float current_disp = disps_[cpv](y, x);
        int nx = x + sign*current_disp;

        float current_cost = costs_[cpv](y, x);

        float new_disp, new_cost;
        if(!inside(nx, y, 0, 0, cols_, rows_))
            return;

        new_disp =  disps_[1-cpv](y, nx);
        new_cost = precomputed_disp_match_cost(new_disp, nx, y, 1-cpv);

        if(new_cost < current_cost)
        {
            disps_[1-cpv](y, nx) = new_disp;
            costs_[1-cpv](y, nx) = new_cost;
        }
    ...
    void PatchMatch::disp_perturbation(int x, int y, int cpv, float max_delta_z, float end_dz) {
        ...
        while(max_dz >= end_dz)
        {
            ...
            float new_disp = current_disp + delta_z;
            float new_cost = precomputed_disp_match_cost(new_disp, x, y, cpv);
